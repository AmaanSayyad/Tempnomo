...
export const BetControls: React.FC<BetControlsProps> = ({
  ...
  const validateBet = (): boolean => {
    ...
    if (amount > houseBalance) {
      setError(`Insufficient house balance. You have ${houseBalance.toFixed(4)} αUSD. Please deposit more.`);
      return false;
    }
      ...
      <div className="space-y-4">
        ...
        {isConnected && (
          <div className="bg-gray-900 rounded p-3">
            ...
            <p className="text-white text-lg font-bold">{houseBalance.toFixed(4)} αUSD</p>
          </div>
        )}

        {/* Bet Amount Input */}
        <div>
          <label className="block text-gray-400 text-sm mb-2 font-mono uppercase tracking-wider">Bet Amount (αUSD)</label>
          <input
            type="number"
            value={betAmount}
        ...
        {selectedTarget && betAmount && parseFloat(betAmount) > 0 && (
          <div className="bg-neon-blue/10 border border-neon-blue/50 rounded p-3 shadow-[0_0_15px_rgba(0,240,255,0.1)]">
            ...
            <p className="text-neon-blue text-2xl font-bold font-mono text-shadow-neon">{potentialPayout} αUSD</p>
          </div>
        )}
