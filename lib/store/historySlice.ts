 ...
 * 
 * Note: After Tempo migration, bet history is managed off-chain.
...
export const createHistorySlice: StateCreator<HistoryState> = (set, get) => ({
  ...
  clearHistory: () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('tempnomo_bet_history');
    }
  },
...
const saveBetsToLocalStorage = (bets: BetRecord[]): void => {
  try {
    if (typeof window !== 'undefined') {
      localStorage.setItem('tempnomo_bet_history', JSON.stringify(bets));
    }
  } catch (error) {
...
const loadBetsFromLocalStorage = (): BetRecord[] => {
  try {
    if (typeof window !== 'undefined') {
      const data = localStorage.getItem('tempnomo_bet_history');
      if (data) {
